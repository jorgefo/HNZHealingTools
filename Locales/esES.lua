local _, ns = ...

-- Traducciones al español (esES). Las claves son el texto en inglés que aparece
-- envuelto en ns.L[...] dentro del código. Si una clave falta aquí, la metatable
-- de Locales.lua devuelve la clave inglesa como fallback, así que añadir entradas
-- aquí no rompe nada — solo añade traducción donde antes se mostraba inglés.
ns.RegisterLocale("esES", {
    -- ===== Mensajes de error / feedback =====
    ["Spell not found: "] = "Hechizo no encontrado: ",
    ["Item not found: "] = "Objeto no encontrado: ",
    [" already monitored."] = " ya está monitorizado.",
    ["Enter a name/ID."] = "Introduce un nombre/ID.",
    ["Enter an item ID, name, or link."] = "Introduce un ID de objeto, nombre o link.",
    ["Enter a name."] = "Introduce un nombre.",
    ["Already exists."] = "Ya existe.",
    ["Created: "] = "Creado: ",
    ["Can't copy to itself."] = "No se puede copiar sobre sí mismo.",
    ["Copied from "] = "Copiado desde ",
    ["Give the profile a name."] = "Ponle un nombre al perfil.",
    ["Paste the exported string in the box."] = "Pega el string exportado en la caja.",
    ["Imported as: "] = "Importado como: ",
    ["Import failed."] = "Import falló.",
    ["Ready: Ctrl+C to copy"] = "Listo: Ctrl+C para copiar",
    ["No active profile to export"] = "No hay perfil activo para exportar",

    -- ===== Botones genéricos =====
    ["Save"] = "Guardar",
    ["Cancel"] = "Cancelar",
    ["Add"] = "Añadir",
    ["Update"] = "Actualizar",
    ["Close"] = "Cerrar",
    ["Edit"] = "Editar",
    ["Test"] = "Probar",
    ["Move up"] = "Mover arriba",
    ["Move down"] = "Mover abajo",
    ["What's New"] = "Novedades",
    ["Got it"] = "Entendido",
    ["Changelog"] = "Registro de cambios",
    ["View release notes for all versions"] = "Ver notas de versión de todas las versiones",

    -- ===== Release notes 1.4.0 =====
    ["Drag trinkets or potions from your bags or equipped slots to the input zone — the addon resolves the use-effect spell ID automatically."] =
        "Arrastra trinkets o pociones desde tu bolsa o slots de equipo a la zona de entrada — el addon resuelve automáticamente el spell ID del use-effect.",
    ["Per-entry visibility for Cursor Spells and Auras: Always / Only in combat / Only out of combat (independent of the global cursor visibility)."] =
        "Visibilidad por entrada para Cursor Spells y Auras: Siempre / Solo en combate / Solo fuera de combate (independiente de la visibilidad global del cursor).",
    ["Per-entry visual overrides for Cursor Spells and Auras: icon size, opacity, and custom position with offset X/Y (the icon detaches from the grid and floats freely)."] =
        "Overrides visuales por entrada para Cursor Spells y Auras: tamaño del icono, opacidad y posición libre con offset X/Y (el icono se separa del grid y flota libremente).",
    ["Tabbed editor modals: Cursor Spell and Cursor Aura split into General / Display / Effects; Ring Aura into General / Effects; Pulse Spell and Pulse Aura into General / Sound."] =
        "Editores con tabs: Cursor Spell y Cursor Aura divididos en General / Display / Effects; Ring Aura en General / Effects; Pulse Spell y Pulse Aura en General / Sound.",
    ["Changelog button (?) in the config window title bar — opens this popup with all release notes on demand."] =
        "Botón Changelog (?) en la barra superior de la ventana de config — abre este popup con todas las notas de versión cuando lo necesites.",
    ["Fix: 'Spell not found' when adding via the autocomplete dropdown for spells/auras the character does not know. The autocomplete-resolved spell ID is now preferred over name lookup."] =
        "Fix: 'Hechizo no encontrado' al agregar desde el autocomplete para hechizos/auras que el personaje no conoce. Ahora se prefiere el spell ID resuelto por el autocomplete sobre el lookup por nombre.",
    ["Fix: creating or switching profiles left some menus showing the old profile's values. Config pages are now rebuilt against the active profile on every switch."] =
        "Fix: al crear o cambiar de perfil, algunos menús quedaban con los valores del perfil anterior. Las páginas de config ahora se reconstruyen contra el perfil activo en cada switch.",
    ["Create"] = "Crear",
    ["Load"] = "Cargar",
    ["Export"] = "Exportar",
    ["Import"] = "Importar",
    ["Copy to current"] = "Copiar al actual",

    -- ===== Pestañas / títulos de página =====
    ["Cursor Spells"] = "Hechizos del cursor",
    ["Cursor Auras"] = "Auras del cursor",
    ["Ring Auras"] = "Auras en anillo",
    ["Cursor Config"] = "Config. cursor",
    ["Ring Config"] = "Config. anillo",
    ["Profiles"] = "Perfiles",

    -- ===== Editores: títulos y labels =====
    ["Cursor Spell"] = "Hechizo del cursor",
    ["Cursor Aura"] = "Aura del cursor",
    ["Ring Aura"] = "Aura en anillo",
    ["New Cursor Spell"] = "Nuevo hechizo del cursor",
    ["New Cursor Aura"] = "Nueva aura del cursor",
    ["New Ring Aura"] = "Nueva aura en anillo",
    ["Editing: "] = "Editando: ",
    ["Spell name or ID:"] = "Nombre del hechizo o ID:",
    ["Aura name or ID:"] = "Nombre del aura o ID:",
    ["Show only when charges >=  (0=always):"] = "Mostrar solo cuando cargas >=  (0=siempre):",
    ["Stack text size (0=default):"] = "Tamaño texto stacks (0=default):",
    ["Hide while on cooldown"] = "Ocultar durante la recuperación",
    ["Hide status overlay"] = "Ocultar overlay de estado",
    ["Hide cooldown / duration timer"] = "Ocultar temporizador de cooldown/duración",
    ["Hide timer"] = "Ocultar temporizador",
    ["Custom icon (file ID):"] = "Icono personalizado (file ID):",
    ["Enter a texture file ID (e.g. 132159). Find IDs on Wowhead — most spell pages link to the icon's file ID. Leave unchecked to use the spell's default icon."] = "Ingresa un file ID de textura (ej. 132159). Búscalos en Wowhead — la mayoría de páginas de hechizo linkean al file ID del icono. Deja desmarcado para usar el icono por defecto del hechizo.",
    ["Specs:"] = "Especializaciones:",
    ["Required talent:"] = "Talento requerido:",
    ["Unit:"] = "Unidad:",
    ["Type:"] = "Tipo:",
    ["Show:"] = "Mostrar:",
    ["Visibility:"] = "Visibilidad:",
    ["Icon size (0=global):"] = "Tamaño icono (0=global):",
    ["Opacity (0=global, 0.1-1):"] = "Opacidad (0=global, 0.1-1):",
    ["Custom position"] = "Posición libre",
    ["Min stacks:"] = "Stacks mín.:",
    ["Duration (sec, 0=auto):"] = "Duración (seg, 0=auto):",
    ["Some auras (item buffs, restricted effects in Midnight) don't expose their duration via API. If the icon/ring shows but doesn't count down, enter the real duration in seconds here (check Wowhead for the exact value)."] = "Algunas auras (especialmente buffs de items consumibles y efectos 'restricted' en Midnight) no exponen su duración via la API. Si el ring/icono se enciende pero no muestra cuenta atrás, ingresá la duración real aquí en segundos (consultá Wowhead para el valor exacto).",
    ["Trigger spell:"] = "Spell disparador:",
    ["Trigger item:"] = "Item disparador:",
    ["Workaround for fully-restricted auras: when the trigger spell is cast OR the trigger item is used, the aura is treated as ACTIVE for the manual duration above. Use only when the standard detection paths fail (verify with /hht auradebug)."] = "Workaround para auras 'fully restricted': cuando se castea el spell disparador O se usa el item disparador, el aura se trata como ACTIVA durante la duración manual indicada arriba. Usar solo cuando los paths de detección estándar fallan (verificá con /hht auradebug).",
    ["Color:"] = "Color:",
    ["Show icon on ring"] = "Mostrar icono en el anillo",
    ["Play sound on activation"] = "Sonar al activarse",
    ["Loop sound"] = "Sonido en bucle",
    ["Loop interval (s):"] = "Intervalo del bucle (s):",
    ["Alert before expiry (s):"] = "Avisar antes de expirar (s):",
    ["Pulse icon at screen center on ready"] = "Icono pulse en el centro al estar listo",
    ["Pulse icon at screen center on activation"] = "Icono pulse en el centro al activarse",
    ["Play sound on ready"] = "Sonar al estar listo",
    ["Pulse"] = "Pulse",
    ["Pulse Display Settings"] = "Ajustes del icono pulse",
    ["Pulse Config"] = "Pulse",
    ["Size & Timing"] = "Tamaño y tiempos",
    ["Hold Duration"] = "Duración",
    ["Enable cooldown pulse"] = "Activar pulse de cooldowns",
    ["Show anchor"] = "Mostrar anclaje",
    ["Hide anchor"] = "Ocultar anclaje",
    ["Test pulse"] = "Probar pulse",

    -- ===== Cursor Ring =====
    ["Cursor Ring"] = "Anillo de cursor",
    ["Cursor Ring Settings"] = "Ajustes del anillo de cursor",
    ["Size & Appearance"] = "Tamaño y apariencia",
    ["Size & Position"] = "Tamaño y posición",
    ["Color"] = "Color",
    ["Ring color"] = "Color del anillo",
    ["Enable cursor ring"] = "Activar anillo de cursor",
    ["Texture"] = "Textura",
    ["Texture & thickness"] = "Textura y grosor",
    ["Each option uses a different stroke width"] = "Cada opción usa un grosor de trazo distinto",
    ["Use class color"] = "Usar color de clase",
    ["Cast progress ring"] = "Anillo de progreso de lanzamiento",
    ["Show cast progress ring"] = "Mostrar anillo de progreso de lanzamiento",
    ["Cast color"] = "Color del lanzamiento",
    ["Cast size"] = "Tamaño del lanzamiento",
    ["Cast opacity"] = "Opacidad del lanzamiento",
    ["Cast direction"] = "Dirección del lanzamiento",
    ["Clockwise (right)"] = "Horario (derecha)",
    ["Counter-clockwise (left)"] = "Antihorario (izquierda)",
    ["Center dot"] = "Punto central",
    ["Show center dot"] = "Mostrar punto central",
    ["Dot color"] = "Color del punto",
    ["Dot size"] = "Tamaño del punto",
    ["Show dot only in combat"] = "Mostrar punto solo en combate",
    ["Grow dot when moving"] = "Agrandar punto al mover el ratón",
    ["Grow scale"] = "Escala al agrandar",

    -- ===== Sidebar / sub-tabs (refactor v1.0.8) =====
    ["Cursor"] = "Cursor",
    ["Ring"] = "Anillo",
    ["Spells"] = "Hechizos",
    ["Auras"] = "Auras",
    ["Config"] = "Ajustes",
    ["Cast"] = "Casteo",
    ["Dot"] = "Punto",

    -- ===== Pulse Spells / Auras (v1.0.9) =====
    ["Pulse Spells"] = "Hechizos pulse",
    ["Pulse Auras"] = "Auras pulse",
    ["Spells that trigger a central icon pulse when their cooldown finishes."] = "Hechizos que disparan un pulse central cuando termina su cooldown.",
    ["Auras that trigger a central icon pulse when gained."] = "Auras que disparan un pulse central al ser ganadas.",
    ["No pulse spells. Add one below."] = "Sin hechizos pulse. Añade uno abajo.",
    ["No pulse auras. Add one below."] = "Sin auras pulse. Añade una abajo.",
    ["Sound"] = "Sonido",
    ["Sound:"] = "Sonido:",
    ["Channel:"] = "Canal:",
    ["Unit:"] = "Unidad:",
    ["Filter:"] = "Filtro:",
    ["Drag to move"] = "Arrastra para mover",
    ["Pulse Spell"] = "Hechizo pulse",
    ["Pulse Aura"] = "Aura pulse",
    ["New Pulse Spell"] = "Nuevo hechizo pulse",
    ["Edit Pulse Spell"] = "Editar hechizo pulse",
    ["New Pulse Aura"] = "Nueva aura pulse",
    ["Edit Pulse Aura"] = "Editar aura pulse",
    ["Add Pulse Spell..."] = "Añadir hechizo pulse...",
    ["Add Pulse Aura..."] = "Añadir aura pulse...",
    ["Play sound on gain"] = "Reproducir sonido al ganarla",

    -- ===== TalentPicker =====
    ["[No talent]"] = "[Ningún talento]",
    ["ID: "] = "ID: ",
    ["Select a talent"] = "Selecciona un talento",
    ["search"] = "buscar",
    ["(no talents in this loadout)"] = "(no hay talentos en este loadout)",

    -- ===== SoundPicker =====
    ["Select a sound"] = "Selecciona un sonido",

    -- ===== DropZone =====
    ["Drag a spell here"] = "Arrastra un hechizo aquí",
    ["Drag a spell or item here"] = "Arrastra un hechizo u objeto aquí",

    -- ===== Dropdowns: unidades / filtros / showWhen =====
    ["Target"] = "Objetivo",
    ["Player"] = "Jugador",
    ["Focus"] = "Foco",
    ["Mouseover"] = "Cursor (mouseover)",
    ["Pet"] = "Mascota",
    ["Buff"] = "Beneficio",
    ["Debuff"] = "Perjuicio",
    ["Always"] = "Siempre",
    ["Only missing"] = "Solo si falta",
    ["Only active"] = "Solo si activa",
    ["Active only"] = "Solo si activa",
    ["Missing only"] = "Solo si falta",
    ["Below stacks"] = "Por debajo de stacks",

    -- ===== Filas de listado (badges) =====
    ["Unknown"] = "Desconocido",
    ["Min:"] = "Mín:",
    ["Hide CD"] = "Sin CD",
    ["Talent"] = "Talento",
    ["ID:"] = "ID:",
    ["s"] = "s",
    ["[icon]"] = "[icono]",
    ["!CDM"] = "!CDM",

    -- ===== Empty-state / hints en páginas =====
    ["No spells. Use 'Add Cursor Spell...' below."] = "Sin hechizos. Usa 'Añadir hechizo de cursor...' abajo.",
    ["No auras. Use 'Add Cursor Aura...' below."] = "Sin auras. Usa 'Añadir aura de cursor...' abajo.",
    ["No ring auras. Use 'Add Ring Aura...' below."] = "Sin auras de anillo. Usa 'Añadir aura de anillo...' abajo.",
    ["Spells shown as icons near the mouse cursor. Click the gear to edit."] = "Hechizos mostrados como iconos junto al cursor. Pulsa el engranaje para editar.",
    ["Auras shown as icons near the mouse cursor. Click the gear to edit."] = "Auras mostradas como iconos junto al cursor. Pulsa el engranaje para editar.",
    ["Auras shown as circular rings around the character. Click the gear to edit, click color to change."] = "Auras mostradas como anillos alrededor del personaje. Pulsa el engranaje para editar, pulsa el color para cambiarlo.",
    ["Add Cursor Spell..."] = "Añadir hechizo de cursor...",
    ["Add Cursor Aura..."] = "Añadir aura de cursor...",
    ["Add Ring Aura..."] = "Añadir aura de anillo...",
    ["or drag a spell here:"] = "o arrastra un hechizo aquí:",
    ["or drag a spell/item here:"] = "o arrastra un hechizo/objeto aquí:",
    ["Drag a spell or item here"] = "Arrastra un hechizo u objeto aquí",
    ["Add Item..."] = "Añadir Objeto...",
    ["Cursor Item"] = "Objeto del cursor",
    ["Pulse Item"] = "Objeto de pulso",
    ["Item ID, name, or link:"] = "ID de objeto, nombre o link:",
    ["Tip: drag the item from your bag/equipment slot directly into the page to auto-fill."] = "Tip: arrastra el objeto desde la mochila/slot de equipo directamente a la página para autocompletar.",
    ["Item"] = "Objeto",
    ["ItemID:"] = "ItemID:",

    -- ===== Filtro por tipo de instancia =====
    ["Show only in:"] = "Mostrar solo en:",
    ["Open World"] = "Mundo abierto",
    ["Delves"] = "Delves",
    ["PvP (Arena/BG)"] = "PvP (Arena/BG)",
    ["Raid"] = "Banda",
    ["Mythic+"] = "Mítica+",
    ["Dungeon"] = "Mazmorra",

    -- ===== Cursor Settings page =====
    ["Cursor Display Settings"] = "Ajustes del display del cursor",
    ["Size & Layout"] = "Tamaño y diseño",
    ["Icon Size"] = "Tamaño icono",
    ["Icon Spacing"] = "Espaciado icono",
    ["Max Columns"] = "Columnas máx.",
    ["Font Size"] = "Tamaño fuente",
    ["Position"] = "Posición",
    ["Offset X"] = "Desplazamiento X",
    ["Offset Y"] = "Desplazamiento Y",
    ["Opacity"] = "Opacidad",
    ["Update Interval"] = "Intervalo de refresco",
    ["Show only in combat"] = "Mostrar solo en combate",
    ["Enable cursor display"] = "Habilitar display del cursor",

    -- ===== Ring Settings page =====
    ["Ring Display Settings"] = "Ajustes del display del anillo",
    ["Size"] = "Tamaño",
    ["Base Radius"] = "Radio base",
    ["Ring Thickness"] = "Grosor del anillo",
    ["Ring Spacing"] = "Espaciado entre anillos",
    ["Appearance"] = "Apariencia",
    ["Segments (smooth)"] = "Segmentos (suavizado)",
    ["Enable ring display"] = "Habilitar display del anillo",
    ["Live preview"] = "Vista previa en vivo",

    -- ===== Profiles page =====
    ["Profile Manager"] = "Gestor de perfiles",
    ["Active: "] = "Activo: ",
    ["(active)"] = "(activo)",
    ["Create New Profile"] = "Crear nuevo perfil",
    ["Copy From Profile"] = "Copiar desde perfil",
    ["Export Current Profile"] = "Exportar perfil actual",
    ["Export Profile"] = "Exportar perfil",
    ["Export current profile..."] = "Exportar perfil actual...",
    ["Import / Export"] = "Importar / Exportar",
    ["Import profile..."] = "Importar perfil...",
    ["(Press Export then Ctrl+C in the box)"] = "(Pulsa Export y luego Ctrl+C en la caja)",
    ["Import Profile"] = "Importar perfil",
    ["Name:"] = "Nombre:",
    ["(paste below and press Import)"] = "(pega abajo y pulsa Import)",

    -- ===== Minimap tooltip =====
    ["Left click:"] = "Click izquierdo:",
    ["Right click:"] = "Click derecho:",
    ["Shift+Right click:"] = "Shift+Click derecho:",
    ["Drag:"] = "Arrastrar:",
    ["open/close config"] = "abrir/cerrar config",
    ["open ready check panel"] = "abrir panel de ready check",
    ["toggle cursor + ring icons"] = "toggle iconos cursor + ring",
    ["move icon"] = "mover icono",

    -- ===== Mensaje de carga =====
    ["loaded"] = "cargado",
    ["Profile:"] = "Perfil:",
    ["Type"] = "Escribe",
    ["for options"] = "para opciones",

    -- ===== General settings =====
    ["General"] = "General",
    ["General Settings"] = "Ajustes generales",
    ["Effects"] = "Efectos",
    -- Pestañas de los editores modales (estilo WeakAuras): Trigger/Display/Load/Actions.
    ["Trigger"] = "Disparador",
    ["Actions"] = "Acciones",
    -- (?) tooltips de los editores modales (una opcion por fila, cada una explicada).
    ["Type the aura's name or its numeric spell ID. Start typing to autocomplete from your spellbook."] = "Escribe el nombre del aura o su ID numérico. Empieza a escribir para autocompletar desde tu libro de hechizos.",
    ["Type the spell's name or its numeric spell ID. Start typing to autocomplete from your spellbook."] = "Escribe el nombre del hechizo o su ID numérico. Empieza a escribir para autocompletar desde tu libro de hechizos.",
    ["Which unit to watch for this aura: target, player, focus, mouseover or pet."] = "Qué unidad vigilar para esta aura: objetivo, jugador, foco, mouseover o mascota.",
    ["Whether to track a Buff (HELPFUL) or a Debuff (HARMFUL). Must match the aura's real type or it won't be found."] = "Si rastrear un beneficio (HELPFUL) o un perjuicio (HARMFUL). Debe coincidir con el tipo real del aura o no se encontrará.",
    ["When to show the icon: Always, only when Missing, only when Active, or when stacks are Below the threshold below."] = "Cuándo mostrar el icono: Siempre, solo cuando Falta, solo cuando Activa, o cuando los stacks estén por Debajo del umbral de abajo.",
    ["When to show the ring: only when Active, Always, only when Missing, or when stacks are Below the threshold below."] = "Cuándo mostrar el anillo: solo cuando Activa, Siempre, solo cuando Falta, o cuando los stacks estén por Debajo del umbral de abajo.",
    ["Used with the 'Below stacks' mode: show the icon while the aura has fewer than this many stacks."] = "Se usa con el modo 'Por debajo de stacks': muestra el icono mientras el aura tenga menos de esta cantidad de acumulaciones.",
    ["Used with the 'Below stacks' mode: show the ring while the aura has fewer than this many stacks."] = "Se usa con el modo 'Por debajo de stacks': muestra el anillo mientras el aura tenga menos de esta cantidad de acumulaciones.",
    ["Manual trigger: when this spell is cast, the aura is treated as ACTIVE for the Duration above. Use only for fully-restricted auras the normal detection can't see (verify with /hht auradebug)."] = "Disparador manual: cuando se lanza este hechizo, el aura se trata como ACTIVA durante la Duración de arriba. Úsalo solo para auras totalmente restringidas que la detección normal no ve (verifica con /hht auradebug).",
    ["Manual trigger: when this item is used, the aura is treated as ACTIVE for the Duration above. Requires Duration > 0."] = "Disparador manual: cuando se usa este objeto, el aura se trata como ACTIVA durante la Duración de arriba. Requiere Duración > 0.",
    ["Override the global icon size for just this aura. 0 = use the global Cursor Display size."] = "Sobrescribe el tamaño de icono global solo para esta aura. 0 = usar el tamaño global del display del cursor.",
    ["Override the global opacity for just this aura (0.1-1). 0 = use the global value."] = "Sobrescribe la opacidad global solo para esta aura (0.1-1). 0 = usar el valor global.",
    ["Override the global icon size for just this spell. 0 = use the global Cursor Display size."] = "Sobrescribe el tamaño de icono global solo para este hechizo. 0 = usar el tamaño global del display del cursor.",
    ["Override the global opacity for just this spell (0.1-1). 0 = use the global value."] = "Sobrescribe la opacidad global solo para este hechizo (0.1-1). 0 = usar el valor global.",
    ["Detach this icon from the grid and place it at a fixed offset from the cursor (set the offsets below)."] = "Desacopla este icono de la grilla y lo coloca en una posición fija respecto al cursor (define los offsets abajo).",
    ["Horizontal offset from the cursor, in pixels. Only used when 'Custom position' is on."] = "Desplazamiento horizontal respecto al cursor, en píxeles. Solo se usa con 'Posición personalizada' activada.",
    ["Vertical offset from the cursor, in pixels. Only used when 'Custom position' is on."] = "Desplazamiento vertical respecto al cursor, en píxeles. Solo se usa con 'Posición personalizada' activada.",
    ["Hide the colored status tint (ready / cooldown / out-of-range) drawn over the icon."] = "Oculta el tinte de estado de color (listo / enfriamiento / fuera de rango) dibujado sobre el icono.",
    ["Hide the countdown / duration text in the center of the icon."] = "Oculta el texto de cuenta atrás / duración en el centro del icono.",
    ["Hide the icon completely while the spell is on cooldown, instead of dimming it."] = "Oculta el icono por completo mientras el hechizo esté en enfriamiento, en vez de atenuarlo.",
    ["Font size of the stack count shown in the icon corner. 0 = use the default size."] = "Tamaño de fuente del contador de stacks en la esquina del icono. 0 = usar el tamaño por defecto.",
    ["Font size of the charge count shown in the icon corner. 0 = use the default size."] = "Tamaño de fuente del contador de cargas en la esquina del icono. 0 = usar el tamaño por defecto.",
    ["When this aura is shown: always, only in combat, or only out of combat."] = "Cuándo se muestra esta aura: siempre, solo en combate, o solo fuera de combate.",
    ["When this icon is shown: always, only in combat, or only out of combat."] = "Cuándo se muestra este icono: siempre, solo en combate, o solo fuera de combate.",
    ["Limit this aura to the selected specs. None or all checked = every spec."] = "Limita esta aura a las especializaciones seleccionadas. Ninguna o todas marcadas = todas las specs.",
    ["Limit this spell to the selected specs. None or all checked = every spec."] = "Limita este hechizo a las especializaciones seleccionadas. Ninguna o todas marcadas = todas las specs.",
    ["Only show when this talent is selected. Leave empty for no talent requirement."] = "Solo mostrar cuando este talento esté seleccionado. Déjalo vacío para no requerir talento.",
    ["Limit this aura to the selected instance types. None or all checked = everywhere."] = "Limita esta aura a los tipos de instancia seleccionados. Ninguno o todos marcados = en todas partes.",
    ["Limit this spell to the selected instance types. None or all checked = everywhere."] = "Limita este hechizo a los tipos de instancia seleccionados. Ninguno o todos marcados = en todas partes.",
    ["Limit this pulse to the selected instance types. None or all checked = everywhere."] = "Limita este pulse a los tipos de instancia seleccionados. Ninguno o todos marcados = en todas partes.",
    ["Briefly flash a large copy of this icon at the center of the screen when the aura becomes active."] = "Muestra brevemente una copia grande de este icono en el centro de la pantalla cuando el aura se activa.",
    ["Briefly flash a large copy of this icon at the center of the screen when the spell comes off cooldown."] = "Muestra brevemente una copia grande de este icono en el centro de la pantalla cuando el hechizo sale de enfriamiento.",
    ["Play a sound when the aura becomes active. Pick the sound below and use Test to preview it."] = "Reproduce un sonido cuando el aura se activa. Elige el sonido abajo y usa Probar para escucharlo.",
    ["Play a sound when the spell comes off cooldown. Pick the sound below and use Test to preview it."] = "Reproduce un sonido cuando el hechizo sale de enfriamiento. Elige el sonido abajo y usa Probar para escucharlo.",
    ["Play a sound when the aura is gained. Pick the sound below and use Test to preview it."] = "Reproduce un sonido cuando se gana el aura. Elige el sonido abajo y usa Probar para escucharlo.",
    ["Sound output channel. Master plays even when in-game sound-effect volume is muted."] = "Canal de salida de sonido. Master suena incluso con el volumen de efectos del juego silenciado.",
    ["Repeat the sound while the aura stays active, at the interval below."] = "Repite el sonido mientras el aura siga activa, en el intervalo de abajo.",
    ["Seconds between repeats while 'Loop sound' is on."] = "Segundos entre repeticiones mientras 'Repetir sonido' esté activado.",
    ["Play the sound and show the timer this many seconds before the aura expires. 0 = off."] = "Reproduce el sonido y muestra el temporizador esta cantidad de segundos antes de que expire el aura. 0 = desactivado.",
    ["Play the sound this many seconds before the aura expires. 0 = off."] = "Reproduce el sonido esta cantidad de segundos antes de que expire el aura. 0 = desactivado.",
    ["Color of this aura's segment on the ring."] = "Color del segmento de esta aura en el anillo.",
    ["Show the aura's icon on its ring segment. Off = a plain colored segment only."] = "Muestra el icono del aura en su segmento del anillo. Desactivado = solo un segmento de color.",
    ["Mouse trail"] = "Estela del cursor",
    ["Mouse trail color"] = "Color de la estela",
    ["Mouse trail length"] = "Longitud de la estela",
    ["Show trail"] = "Mostrar estela",
    ["Show grow"] = "Mostrar agrandar",
    ["Restore from backup"] = "Restaurar desde backup",
    ["Restore"] = "Restaurar",
    ["(no backups)"] = "(sin backups)",
    ["Restore failed."] = "Restauración falló.",
    ["Restored profile"] = "Perfil restaurado",
    ["Type /reload to fully apply old field formats."] = "Escribe /reload para aplicar correctamente los campos antiguos.",
    ["MRT"] = "MRT",
    ["MRT / NSRT Timeline Reminders"] = "Recordatorios de timeline MRT / NSRT",
    ["Enable MRT / NSRT timeline"] = "Habilitar timeline MRT / NSRT",
    ["Enable"] = "Habilitar",
    ["Display"] = "Visualización",
    ["Lead time (s)"] = "Anticipación (s)",
    ["Linger after (s)"] = "Persistir después (s)",
    ["Test pull"] = "Probar pull",
    ["Entries for you"] = "Entradas para ti",
    ["Active window (s)"] = "Ventana activa (s)",
    ["Ring icon size"] = "Tamaño icono del anillo",
    ["Encounter ID:"] = "ID del encuentro:",
    ["Difficulties:"] = "Dificultades:",
    ["LFR"] = "BdB",
    ["Normal"] = "Normal",
    ["Heroic"] = "Heroico",
    ["Mythic"] = "Mítico",
    ["Select at least one difficulty."] = "Selecciona al menos una dificultad.",
    ["Delete"] = "Eliminar",
    ["Encounters"] = "Encuentros",
    ["View"] = "Ver",
    ["Note entries"] = "Entradas de la nota",
    ["No entries match your player name."] = "Ninguna entrada coincide con tu nombre.",
    ["Import note..."] = "Importar nota...",
    ["Import note"] = "Importar nota",
    ["Edit note"] = "Editar nota",
    ["MRT / NSRT note editor"] = "Editor de nota MRT / NSRT",
    ["Format:"] = "Formato:",
    ["Save"] = "Guardar",
    ["(paste below)"] = "(pegar abajo)",
    ["Paste the note text first."] = "Pega el texto de la nota primero.",
    ["Display configuration"] = "Configuración de visualización",
    ["No notes. Use 'Import note...' below."] = "Sin notas. Usa 'Importar nota...' abajo.",
    ["NSRT: ID and Name are auto-detected from the header. You can override below — type a boss name to search."] = "NSRT: el ID y Nombre se autodetectan desde el header. Podés sobreescribir — escribí el nombre del jefe para buscar.",
    ["MRT: paste the note. Type the boss name in the ID field to search, or enter ID 0 for any encounter."] = "MRT: pega la nota. Escribí el nombre del jefe en el campo ID para buscar, o ingresá ID 0 para cualquier encuentro.",
    ["MRT / NSRT"] = "MRT / NSRT",
    ["Show MRT/NSRT triggers"] = "Mostrar disparadores de MRT/NSRT",
    ["Fixed position"] = "Posición fija",
    ["Show at fixed screen position"] = "Mostrar en posición fija de pantalla",
    ["Move"] = "Mover",
    ["Lock"] = "Bloquear",
    ["Hide countdown (fixed)"] = "Ocultar contador (fijo)",
    ["Fixed icon size (0=global):"] = "Tamaño icono fijo (0=global):",
    ["Countdown size (0=default):"] = "Tamaño contador (0=defecto):",
    -- Secciones por display en MRT/NSRT → Config (rotular qué configura qué)
    ["Timing (all displays)"] = "Tiempos (todos los displays)",
    ["Cursor icons"] = "Iconos del cursor",
    ["Show as cursor icons"] = "Mostrar como iconos del cursor",
    ["Ring (character)"] = "Anillo (personaje)",
    ["Show as character ring"] = "Mostrar como anillo del personaje",
    ["Show as central pulse"] = "Mostrar como pulse central",
    ["Pulse visuals (size/animation) are configured in the Pulse menu."] = "Los visuales del pulse (tamaño/animación) se configuran en el menú Pulse.",
    ["Fixed icon (on screen)"] = "Icono fijo (en pantalla)",
    -- Fixed Panels (paneles fijos de iconos: misma logica del cursor, anclados)
    ["Fixed Panels"] = "Paneles fijos",
    ["Fixed Panel"] = "Panel fijo",
    ["Panel name"] = "Nombre del panel",
    ["Panel"] = "Panel",
    ["Add Panel"] = "Agregar panel",
    ["No panels. Use 'Add Panel' below."] = "Sin paneles. Usá 'Agregar panel' abajo.",
    ["Icons anchored to fixed screen spots, grouped in named panels. Same tracking as the cursor, but they don't follow the mouse."] = "Iconos anclados a puntos fijos de pantalla, agrupados en paneles con nombre. Mismo seguimiento que el cursor, pero no siguen al mouse.",
    ["Tip: use Move to drag the panel on screen."] = "Tip: usá Mover para arrastrar el panel en pantalla.",
    ["Show:"] = "Mostrar:",
    -- CDM Enhancer (efectos sobre los iconos del Cooldown Manager de Blizzard)
    ["CDM Enhancer"] = "Mejorar CDM",
    ["CDM Rule"] = "Regla CDM",
    ["Add Rule"] = "Agregar regla",
    ["No rules. Use 'Add Rule' below."] = "Sin reglas. Usá 'Agregar regla' abajo.",
    ["Enhance icons already shown by Blizzard's Cooldown Manager. Effects only apply to spells you've added to the CDM (Edit Mode)."] = "Mejora los iconos que ya muestra el Cooldown Manager de Blizzard. Los efectos solo aplican a hechizos que agregaste al CDM (Edit Mode).",
    ["When about to expire"] = "Cuando está por expirar",
    ["Alert seconds before expiry"] = "Avisar segundos antes de expirar",
    ["When ready (off cooldown)"] = "Cuando está listo (sin enfriamiento)",
    ["When below stacks / missing"] = "Cuando falta o baja de stacks",
    ["Min stacks"] = "Stacks mínimos",
    ["Glow"] = "Resplandor",
    ["Grow / pulse"] = "Agrandar / latir",
    ["Pulse scale"] = "Escala del latido",
    ["Recolor border"] = "Recolorear borde",
    ["Desaturate icon"] = "Desaturar icono",
    ["Show countdown"] = "Mostrar cuenta atrás",
    ["Spell:"] = "Hechizo:",
    ["Play sound on trigger"] = "Reproducir sonido al disparar",
    ["Voice (TTS)"] = "Voz (TTS)",
    ["Announce spell name by voice"] = "Anunciar nombre del hechizo por voz",
    ["TTS lead time (s)"] = "Anticipación de voz (s)",
    ["TTS volume"] = "Volumen de voz",
    ["Voice:"] = "Voz:",
    ["Language:"] = "Idioma:",
    ["Auto (client locale)"] = "Auto (idioma del cliente)",
    ["Apply"] = "Aplicar",
    ["Sparkle effect"] = "Destellos",
    ["Sparkle color"] = "Color del destello",
    ["Sparkle size"] = "Tamaño del destello",
    ["Sparkle shape"] = "Forma del destello",
    ["Show sparkles"] = "Mostrar destellos",
    ["Only in combat"] = "Solo en combate",
    ["Only out of combat"] = "Solo fuera de combate",
    ["Dot"] = "Punto",
    ["Ring (thin)"] = "Anillo fino",
    ["Ring (thick)"] = "Anillo grueso",
    ["Wedge"] = "Cuña",
    ["Mixed"] = "Mezclado",
    ["Language:"] = "Idioma:",
    ["Detected client:"] = "Cliente detectado:",
    ["Most labels are read once when the panel is built. Reload UI to refresh all text."] = "La mayoría de los textos se leen una sola vez al abrir el panel. Recarga la UI para refrescarlos todos.",
    ["Reload UI"] = "Recargar UI",

    -- ===== Talent tree labels (devueltos por Utils.GetClassTalents/GetPlayerSpells) =====
    ["Class"] = "Clase",
    ["Hero"] = "Héroe",
    -- "Spec" se mantiene igual (palabra ya común en español); si el usuario quiere cambiarla, aquí.

    -- ===== Release notes 1.5.0 =====
    ["Track items as cooldowns: trinkets, potions and on-use consumables can now be added to the Cursor or Pulse list. New 'Add Item...' button + drag-and-drop dispatches by type (spell vs item) and opens the right editor."] =
        "Trackear items como cooldowns: trinkets, pociones y consumibles on-use ahora pueden agregarse a la lista Cursor o Pulse. Nuevo botón 'Add Item...' + drag-and-drop despacha por tipo (spell vs item) y abre el editor correcto.",
    ["Item editors with full tabs (mirror of the Spell editor): General + Display + Effects for cursor items; General + Sound for pulse items. Visual overrides, hide flags, pulse on ready, sound — all available."] =
        "Editores de items con tabs completos (mirror del editor de Spell): General + Display + Effects para items del cursor; General + Sound para items del pulse. Overrides visuales, hide flags, pulse al estar listo, sonido — todo disponible.",
    ["Per-entry instance-type filter on every aura/spell/item editor: restrict tracking to Open World, Delves, PvP (Arena/BG), Raid, Mythic+ and/or Dungeon. Reacts instantly when entering/leaving instances."] =
        "Filtro per-entry por tipo de instancia en cada editor de aura/spell/item: restringe el tracking a Mundo abierto, Delves, PvP (Arena/BG), Banda, Mítica+ y/o Mazmorra. Reacciona al instante al entrar/salir de instancias.",
    ["Aura detection paths 6 + 7: slot iteration (catches semi-restricted auras Midnight hides from name/ID lookups) + manual trigger workaround (for fully-restricted auras like consumable buffs — configure a trigger spell or item ID and the addon synthesizes the ACTIVE state on cast/use)."] =
        "Paths 6 + 7 de detección de auras: iteración de slots (atrapa auras semi-restringidas que Midnight oculta a los lookups por nombre/ID) + workaround de manual trigger (para auras fully-restricted como buffs de consumibles — configurás un spell o item disparador y el addon sintetiza el estado ACTIVE al castear/usar).",
    ["New /hht listauras command: prints every active buff/debuff with name + spellID + source + duration. Useful for finding the real spellID of a buff when the guessed one isn't detected."] =
        "Nuevo comando /hht listauras: imprime todos los buffs/debuffs activos con nombre + spellID + source + duración. Útil para encontrar el spellID real de un buff cuando el adivinado no se detecta.",
    ["Config window no longer closes when opening the Spellbook (PlayerSpellsFrame). ESC still closes it via a custom handler that doesn't break other keybinds."] =
        "La ventana de config ya no se cierra al abrir el Libro de hechizos (PlayerSpellsFrame). ESC sigue cerrándola via un handler propio que no rompe otros keybinds.",
    ["Fix: comparing SecureNumber spellId in slot iteration tainted the addon ('attempt to compare a secret number value'). Wrapped in ToPublic + pcall — fully restricted auras are skipped safely instead of crashing the whole frame."] =
        "Fix: comparar spellId SecureNumber en la iteración de slots tainteaba el addon ('attempt to compare a secret number value'). Envuelto en ToPublic + pcall — auras fully-restricted se saltean a salvo en lugar de crashear el frame entero.",
    ["Fix: ApplyRingVisibility nil call when a ring test entry expired (forward declaration bug, latent since 1.3.0)."] =
        "Fix: llamada nil a ApplyRingVisibility cuando una entry de test del ring expiraba (bug de forward declaration, latente desde 1.3.0).",

    -- ===== Release notes 1.6.0 =====
    ["Macro trigger system: every aura, pulse, and item editor has a new 'Trigger key' field. Fire any configured display from a macro with /hht trigger <key> or from another addon via HNZHealingTools.Trigger(key). Multiple entries can share a key — one keybind fires them all at once."] =
        "Sistema de trigger por macro: cada editor de aura, pulse e item tiene un campo nuevo 'Trigger key'. Disparás cualquier display configurado desde una macro con /hht trigger <key> o desde otro addon vía HNZHealingTools.Trigger(key). Varias entries pueden compartir la misma key — un solo keybind las dispara todas.",
    ["New Macros help page in the config sidebar with copy-pasteable macro examples and Lua snippets."] =
        "Nueva página Macros en el sidebar del config con ejemplos de macro y snippets de Lua copiables.",
    ["Floating preview popup: 'Show preview' button at the top of pages with a Live Preview block (Cursor / Ring / Pulse settings + Cursor Ring sub-tabs). Opens to the right of the config window, single-active across pages, inherits position when switching."] =
        "Preview popup flotante: botón 'Show preview' arriba de las páginas que tenían preview inline (Cursor / Ring / Pulse settings + sub-tabs de Cursor Ring). Se abre a la derecha del config, único activo entre páginas, hereda la posición al cambiar.",
    ["Stack count now displays correctly for fully-restricted auras tracked by Blizzard's Cooldown Manager (e.g. Mana Tea). The addon now reads the stack count via the same SetText/GetText technique Blizzard's own CDM viewer uses, so SecureNumber values are no longer lost in combat."] =
        "El número de cargas ahora se muestra correctamente para auras fully-restricted trackeadas por el Cooldown Manager de Blizzard (e.g. Té de maná). El addon lee el conteo vía la misma técnica SetText/GetText que usa el viewer CDM de Blizzard, así los valores SecureNumber no se pierden en combate.",
    ["Restricted auras visible in the Cooldown Manager but invisible to addon APIs now synthesize ACTIVE state from the CDM hook (stacks + appliedAt) — icon + count + optional timer render correctly even when all 6 detection paths fail."] =
        "Auras restricted visibles en el Cooldown Manager pero invisibles a la API del addon ahora sintetizan estado ACTIVE desde el hook de CDM (stacks + appliedAt) — icono + conteo + timer opcional se renderizan aunque los 6 paths de detección fallen.",
    ["/hht auradebug now reports inCombat status, CDM-captured stack count, and the full list of FontStrings on the matching CDM frame — useful for diagnosing in-combat detection failures."] =
        "/hht auradebug ahora reporta estado de combate (inCombat), conteo de stacks capturado por CDM, y la lista completa de FontStrings del frame CDM que matchea — útil para diagnosticar fallos de detección en combate.",
    ["Public API namespace _G.HNZHealingTools exposed for macros and other addons (.version, .Trigger(key))."] =
        "Namespace de API pública _G.HNZHealingTools expuesto para macros y otros addons (.version, .Trigger(key)).",

    -- ===== Macros page + trigger UI =====
    ["Macros"] = "Macros",
    ["Trigger key:"] = "Clave trigger:",
    ["Show preview"] = "Ver preview",
    ["Hide preview"] = "Ocultar preview",
    ["Miscellaneous"] = "Varios",
    ["Combat Resurrections"] = "Resurrecciones de combate",
    ["Enable combat resurrection indicator"] = "Activar indicador de resurrecciones de combate",
    ["Text size (0=auto)"] = "Tamaño de texto (0=auto)",
    ["Move icon"] = "Mover icono",
    ["Lock icon"] = "Fijar icono",
    ["Right-click: open settings (out of combat)"] = "Click derecho: abrir configuración (fuera de combate)",
    ["Show Reincarnation (Shaman self-res)"] = "Mostrar Reencarnación (auto-res de Chamán)",
    ["Shows the group's shared battle-res charges (raid / Mythic+) — it does not depend on your class. With charges available: the number. Without: the icon dims and counts down to the next charge. Hidden in content with no shared pool."] =
        "Muestra las cargas de resurrección de combate compartidas del grupo (raid / Mítica+) — no depende de tu clase. Con cargas disponibles: el número. Sin cargas: el icono se apaga y muestra la cuenta regresiva hasta la próxima carga. Oculto en contenido sin pool compartido.",
    ["Optional. Fire this aura from a macro: /hht trigger <key>. Requires Duration > 0. Case-insensitive."] =
        "Opcional. Disparar esta aura desde una macro: /hht trigger <key>. Requiere Duration > 0. Insensible a mayúsculas.",
    ["Optional. Fire this pulse from a macro: /hht trigger <key>. Case-insensitive."] =
        "Opcional. Disparar este pulse desde una macro: /hht trigger <key>. Insensible a mayúsculas.",
    ["Optional. Fire this item from a macro: /hht trigger <key>. Case-insensitive."] =
        "Opcional. Disparar este item desde una macro: /hht trigger <key>. Insensible a mayúsculas.",
    ["Usage: /hht trigger <key>"] = "Uso: /hht trigger <key>",
    ["Triggered %d entrie(s) for key '%s'"] = "Disparadas %d entries para la key '%s'",
    ["No entries match triggerKey '%s'"] = "Ninguna entry coincide con triggerKey '%s'",
    ["Trigger displays from macros"] = "Disparar displays desde macros",
    ["You can fire any aura or pulse from a macro, keybind, or another addon — without needing the actual aura/cooldown to trigger. Useful for one-shot visual signals (panic ring, cooldown reminder, callout from a partner addon)."] =
        "Podés disparar cualquier aura o pulse desde una macro, keybind u otro addon — sin necesidad de que la aura/cooldown se active realmente. Útil para señales visuales puntuales (ring de pánico, recordatorio de cooldown, aviso desde otro addon).",
    ["1. Where you can set a Trigger key"] = "1. Dónde podés setear una Trigger key",
    ["Open the editor of any of these and fill in the \"Trigger key\" field:"] =
        "Abrí el editor de cualquiera de estos y llená el campo \"Trigger key\":",
    ["  • Cursor Aura — fires the aura's icon at cursor for its Duration."] =
        "  • Cursor Aura — dispara el icono del aura cerca del cursor durante su Duration.",
    ["  • Ring Aura — fires the colored ring around your character for its Duration."] =
        "  • Ring Aura — dispara el anillo de color alrededor del personaje durante su Duration.",
    ["  • Cursor Item — fires the central pulse with the item's icon + optional sound."] =
        "  • Cursor Item — dispara el pulse central con el icono del item + sonido opcional.",
    ["  • Pulse Spell / Pulse Aura / Pulse Item — fires the central screen pulse + optional sound."] =
        "  • Pulse Spell / Pulse Aura / Pulse Item — dispara el pulse central de pantalla + sonido opcional.",
    ["2. Fire it"] = "2. Disparalo",
    ["From a chat message or macro line:"] = "Desde un mensaje de chat o línea de macro:",
    ["From Lua (other addons, /run, WeakAuras custom code):"] =
        "Desde Lua (otros addons, /run, código custom de WeakAuras):",
    ["Example: cast + trigger together"] = "Ejemplo: cast + trigger juntos",
    ["Combine a real cast with a visual trigger in one macro:"] =
        "Combiná un cast real con un trigger visual en una sola macro:",
    ["Tips"] = "Tips",
    ["  • Multiple entries can share the same trigger key — they all fire at once (e.g. one key can show a Ring Aura + play a Pulse simultaneously)."] =
        "  • Varias entries pueden compartir la misma trigger key — se disparan todas a la vez (e.g. una key puede mostrar un Ring Aura + tocar un Pulse simultáneamente).",
    ["  • Trigger keys are case-insensitive. \"Panic\" and \"panic\" match the same entries."] =
        "  • Las trigger keys son insensibles a mayúsculas. \"Panic\" y \"panic\" matchean las mismas entries.",
    ["  • Aura entries (Cursor / Ring) require Duration > 0 — without a duration there's no way to know when the visual should disappear."] =
        "  • Las entries de aura (Cursor / Ring) requieren Duration > 0 — sin duración no hay forma de saber cuándo desaparece el visual.",
    ["  • Pulse entries fire immediately and the animation has its own length (configured globally in Pulse → Config)."] =
        "  • Las entries de pulse se disparan al instante y la animación tiene su largo propio (configurado globalmente en Pulse → Config).",
    ["  • HNZHealingTools.Trigger(key) returns the number of entries that matched (0 = no entries have that key)."] =
        "  • HNZHealingTools.Trigger(key) devuelve la cantidad de entries que matchearon (0 = ninguna entry tiene esa key).",
    ["  • Combat-safe: trigger keys work during combat lockdown."] =
        "  • Combat-safe: las trigger keys funcionan durante combat lockdown.",

    -- ===== Ready Check Panel (1.7.0) =====
    ["Ready Check"]                  = "Ready Check",
    ["Ready Check Panel"]            = "Panel de Ready Check",
    ["Enable ready check panel"]     = "Activar panel de Ready Check",
    ["Size & Appearance"]            = "Tamaño y aspecto",
    ["Panel Width"]                  = "Ancho del panel",
    ["Row Height"]                   = "Alto de fila",
    ["Font Scale"]                   = "Escala de texto",
    ["more"]                         = "más",
    ["Items to check"]               = "Items a verificar",
    ["Talents"]                      = "Talentos",
    ["Show talents banner"]          = "Mostrar banner de talentos",
    ["Well Fed"]                     = "Bien Alimentado",
    ["Flask / Phial"]                = "Frasco / Filtro",
    ["Augment Rune"]                 = "Runa de aumento",
    ["Check your mana"]              = "Revisa tu maná",
    ["Class buffs (party-aware)"]    = "Bufos de clase (según grupo)",
    ["Weapon Imbue"]                 = "Encantamiento de arma",
    ["Healthstone"]                  = "Piedra de salud",
    ["General"]                      = "General",
    ["Talents"]                      = "Talentos",
    ["Enabled menus"]                = "Menús habilitados",
    ["Features"]                     = "Funciones",
    ["Enable features"]              = "Habilitar Funciones",
    ["Feature disabled — enable in General > Habilitar Funciones"] =
        "Función deshabilitada — actívala en General > Habilitar Funciones",
    ["Current spec"]                 = "Especialización activa",
    ["Refresh"]                      = "Refrescar",
    ["No saved loadouts for this spec"] = "Sin loadouts guardados para esta especialización",
    ["Select the content type(s) where each loadout should be used. The panel warns at ready check if you're on the wrong one."] =
        "Selecciona en qué tipo(s) de contenido se debe usar cada loadout. El panel avisa en el ready check si no estás en el correcto.",
    ["Wrong loadout"]                = "Loadout incorrecto",
    ["Wrong talent build"]           = "Talento incorrecto",
    ["Switch"]                       = "Cambiar",
    ["Loadout"]                      = "Loadout",
    ["Talent Build"]                 = "Talentos",
    ["Can't switch loadout in combat."] = "No se puede cambiar de loadout en combate.",
    ["Raid"]                         = "Banda",
    ["Mythic+"]                      = "Mítica+",
    ["Dungeon"]                      = "Mazmorra",
    ["Dungeon / M+"]                 = "Mazmorra / M+",
    ["PvP"]                          = "JcJ",
    ["Delve"]                        = "Expedición",
    ["No loadout assigned for %s"]   = "Sin loadout asignado para %s",
    ["Cast"]                         = "Lanzar",
    ["Ask"]                          = "Pedir",
    ["Use"]                          = "Usar",
    ["(eating)"]                     = "(comiendo)",
    ["No items in bag"]              = "Sin items en la mochila",
    -- Titulos de secciones grid en el panel de Ready Check (titulo centrado
    -- arriba de cada fila horizontal de iconos + boton Pedir/Lanzar/Usar).
    ["Class buffs"]                  = "Bufos de clase",
    ["Foods"]                        = "Comidas",
    ["Flasks"]                       = "Frascos",
    ["Augment Runes"]                = "Runas de aumento",
    ["Weapon Oils"]                  = "Aceites de arma",
    -- NOTA: los labels de los class buffs (Skyfury -> Abrasacielos, etc.) se
    -- resuelven en runtime via C_Spell.GetSpellInfo(spellID).name, asi que NO
    -- necesitamos traducirlos aqui. WoW devuelve el nombre oficial del juego
    -- en el idioma del cliente. Las entries Power Word: Fortitude / Arcane
    -- Intellect / Battle Shout / Mark of the Wild / Skyfury / Blessing of the
    -- Bronze solo se usan como fallback enUS si la API falla.
    ["Test panel"]                   = "Probar panel",
    ["Buff detection currently assumes English buff names. Localized clients may see false negatives."] =
        "La detección de buffs asume nombres en inglés. Los clientes localizados pueden ver falsos negativos.",

    -- ===== Auction House (AuctionRestock module) =====
    -- Sidebar label + header de la pagina.
    ["Auction House"]                = "Subasta",
    ["Auction Restock"]              = "Comprar en Subasta",
    ["Auction Item"]                 = "Objeto de Subasta",

    -- Labels del boton flotante (texto principal y subtexto).
    ["Restock"]                      = "Comprar",
    ["Configure restock"]            = "Configurar compras",
    ["click to open config"]         = "click para abrir config",
    ["All stocked"]                  = "Todo abastecido",
    ["Nothing on AH"]                = "Nada en la subasta",
    ["Searching..."]                 = "Buscando...",
    ["Price too high"]               = "Precio muy alto",
    ["item types"]                   = "tipos de objeto",
    ["too pricey"]                   = "muy caro",
    ["unavail"]                      = "no disp.",
    ["pending"]                      = "pendiente",

    -- Status labels en el tooltip (claves vienen de STATUS_LABEL_KEY).
    -- "OK" se deja literal porque es universal.
    ["price > max"]                  = "precio > máx",
    ["not on AH"]                    = "no en subasta",
    ["searching..."]                 = "buscando...",
    ["queued"]                       = "en cola",

    -- Tooltips y mensajes operacionales del boton.
    ["No items configured yet. Click to open Config."] =
        "Sin objetos configurados. Click para abrir Config.",
    ["Shift+Drag to move."]          = "Shift+Arrastrar para mover.",
    ["Drag to move."]                = "Arrastra para mover.",
    ["All items at or above target. Nothing to restock."] =
        "Todos los objetos alcanzaron el objetivo. Nada que comprar.",
    ["Click to refresh search & jump to the first item. Buy via standard AH UI."] =
        "Click para refrescar búsqueda y saltar al primer objeto. Compra desde la UI estándar de la subasta.",
    ["Open Config > Auction House to add items."] =
        "Abre Config > Subasta para añadir objetos.",
    ["Open the auction house first."] = "Abre la casa de subastas primero.",
    ["Another purchase already in progress."] = "Ya hay otra compra en curso.",
    ["No items available to auto-buy right now."] =
        "No hay objetos disponibles para auto-comprar ahora mismo.",
    [" already in restock list."]    = " ya está en la lista de compras.",

    -- Panel "Shopping list" (toggle al hacer click en el boton flotante).
    ["Shopping list"]                = "Lista de compras",
    ["Buy"]                          = "Comprar",
    ["Buy all"]                      = "Comprar todo",
    ["Buying..."]                    = "Comprando...",
    ["Total:"]                       = "Total:",
    -- Indicadores de mail: cuando los items ya fueron comprados y estan
    -- esperando en el correo, sumamos a un cache per-character escaneado en
    -- MAIL_INBOX_UPDATE y los descontamos del 'need'.
    ["in mail"]                      = "en correo",
    ["bag"]                          = "mochila",
    ["mail"]                         = "correo",
    ["Already bought (in mail)"]     = "Ya comprado (en correo)",
    ["mail scan:"]                   = "buzón escaneado hace:",
    ["mail not scanned — open the mailbox"] =
        "buzón sin escanear — abre el correo",
    ["Check your mail"]              = "Revisa el correo",

    -- Mensajes del flujo de compra (cotizacion / popup / resultado).
    ["Quoting"]                      = "Cotizando",
    ["Purchase complete"]            = "Compra completada",
    ["Price changed"]                = "El precio cambió",
    ["max"]                          = "máx",
    ["Buy %d x %s for %s (%s/ea)?"]  = "¿Comprar %d x %s por %s (%s/ud)?",
    -- Popup de confirmacion detallado (BuildBuyConfirmText).
    ["In bag"]                       = "En mochila",
    ["target"]                       = "objetivo",
    ["Previous"]                     = "Anterior",
    ["same"]                         = "igual",
    ["No previous purchase recorded."] = "Sin compras anteriores registradas.",
    ["Price spike — verify before buying."] =
        "Subida de precio fuerte — verifica antes de comprar.",

    -- Pagina de configuracion: descripcion + campos del editor.
    ["When you open the auction house, a button appears showing current prices for these items. Click to trigger a search; buy via the standard AH UI. Max price per item caps purchases; lastPaid tracks price changes."] =
        "Cuando abres la casa de subastas, aparece un botón mostrando los precios actuales de estos objetos. Click para buscar; compra desde la UI estándar de la subasta. El precio máximo por unidad limita las compras; lastPaid registra los cambios de precio.",
    ["How many to buy:"]             = "Cuántos comprar:",
    ["Target in bag"]                = "Objetivo en mochila",
    ["Max price per item (gold, 0 = no limit):"] =
        "Precio máximo por unidad (oro, 0 = sin límite):",
    ["Confirm above (gold):"]        = "Confirmar si supera (oro):",
    ["Confirm above (gold, 0 = no confirmation):"] =
        "Confirmar si supera (oro, 0 = sin confirmación):",
    ["Global confirm threshold (gold, 0 = off):"] =
        "Umbral global de confirmación (oro, 0 = desactivado):",
    ["If the total cost exceeds this amount, a confirmation popup appears even when the per-item threshold doesn't trigger it."] =
        "Si el costo total supera este monto, aparece un popup de confirmación aunque el umbral per-item no lo dispare.",
    ["Button settings"]              = "Ajustes del botón",
    ["or drag an item here:"]        = "o arrastra un objeto aquí:",

    -- ===== Raid Healer Comms =====
    ["Raid Spells"]                  = "Raid Hechizos (A)",
    ["Waiting for healer casts..."]  = "Esperando casts de healers...",
    ["Broadcasts your important healer cooldown casts to other raid members running HNZ Healing Tools, and shows a panel of what your fellow healers cast in the last 5 seconds."] =
        "Difunde tus cooldowns de healer importantes al resto del grupo que use HNZ Healing Tools, y muestra un panel con lo que tus compañeros healers lanzaron en los últimos 5 segundos.",
    ["Hide panel when no casts"]     = "Ocultar panel cuando no hay casts",
    ["Show only on healer specs"]    = "Mostrar solo en specs healer",
    ["Show only in raid"]            = "Mostrar solo en raid",
    ["Reset position"]               = "Resetear posición",
    ["Tracked spells"]               = "Hechizos rastreados",
    ["Add by spell ID or exact name. Reset restores the curated default set (and re-picks up any new defaults added in future updates)."] =
        "Añade por ID de hechizo o nombre exacto. Reset restaura el set curado por defecto (y recupera cualquier nuevo default añadido en futuras actualizaciones).",
    ["Spell ID or name"]             = "ID o nombre del hechizo",
    ["Reset to defaults"]            = "Restaurar valores por defecto",
    ["Reset to defaults."]           = "Restaurado a valores por defecto.",
    ["Enter a spell ID or name."]    = "Introduce un ID o nombre de hechizo.",
    ["Unknown spell — check the ID or name."] =
        "Hechizo desconocido — revisa el ID o el nombre.",
    ["Spell already tracked."]       = "Ese hechizo ya está rastreado.",
    ["Added: %s"]                    = "Añadido: %s",
    ["Removed: %s"]                  = "Eliminado: %s",
    ["Healer comms test preview shown. Click Hide to clear."] =
        "Vista previa de Raid Hechizos Alpha mostrada. Click en Ocultar para limpiar.",

    -- ===== Simulated Auras =====
    ["Simulated Auras"]              = "Hechizos simulados (A)",
    ["Curated list of spells whose aura the WoW API hides from addons. Enable a preset to track it as 'apply N stacks on cast, consume 1 per re-press'. Each preset includes the macro you need to copy into your action bar."] =
        "Lista curada de hechizos cuya aura la API de WoW oculta a los addons. Habilita un preset para trackearlo como 'aplicar N stacks al castear, consumir 1 por cada press'. Cada preset incluye la macro que necesitas pegar en tu barra de acción.",
    ["Stacks"]                       = "Cargas",
    ["Duration"]                     = "Duración",
    ["Enable"]                       = "Habilitar",
    ["Disable"]                      = "Deshabilitar",
    ["Instructions"]                 = "Instrucciones",
    ["Hide instructions"]            = "Ocultar instrucciones",
    ["Steps"]                        = "Pasos a seguir",
    ["Macro (CTRL+C to copy)"]       = "Macro (CTRL+C para copiar)",
    ["Close"]                        = "Cerrar",
    ["No presets available for your class yet."] = "No hay presets disponibles para tu clase todavía.",
    ["Status: disabled"]             = "Estado: deshabilitado",
    ["Status: enabled, waiting for cast"] = "Estado: habilitado, esperando cast",
    ["Status: active %d/%d cargas%s"] = "Estado: activo %d/%d cargas%s",
    ["Test press fired: %s"]         = "Press de prueba disparado: %s",
    ["Coming soon"]                  = "Próximamente",
    ["A curated list of preset spells whose aura the WoW API hides from addons. Each preset will include the macro to copy into your action bar. The state machine is functional via the /hnzsim slash command but the UI integration is still in testing."] =
        "Lista curada de hechizos cuya aura la API de WoW oculta a los addons. Cada preset incluirá la macro lista para copiar en tu barra de acción. El state machine ya funciona vía el comando /hnzsim pero la integración con el display sigue en pruebas.",
    ["Enabled: %s"]                  = "Habilitado: %s",
    ["Disabled: %s"]                 = "Deshabilitado: %s",
    ["No simulated auras configured."] = "No hay hechizos simulados configurados.",
    ["Configured simulated auras:"]  = "Hechizos simulados configurados:",

    -- Genericos compartidos con AuctionRestock que faltaban en otros idiomas.
    ["Hide"]                         = "Ocultar",
    ["Enabled"]                      = "Activado",
    ["Scale"]                        = "Escala",
})
